...
export default class Lobby extends Phaser.Scene {
  ...
  createCharacter() {
    ...
    this.add.sprite(560, 290, "stone1").setOrigin(0.5, 1).setDisplaySize(100, 80).setDepth(0);
    this.add.sprite(660, 350, "stone1").setOrigin(0.5, 1).setDisplaySize(110, 80).setDepth(2);
  ...
  createButtons() {
    ...
    for (let i = 0; i < lastStage + 1; i++) {
      ...
      const button = new Phaser.GameObjects.Sprite(this, 50 + 200 * i, 365, "stage");
      ...
      addButtonEvent(button, () => {
        const games = [SCENE.SHOOTING_GAME, SCENE.PUZZLE_GAME, SCENE.MATCHING_GAME];
    ...
    for (let i = lastStage + 1; i < GAME.TOTAL_STAGE; i++) {
      const lockedButton = new Phaser.GameObjects.Sprite(this, 200 * i + 50, 365, "lock");
  ...
  createClickGuide() {
    ...
    this.add.sprite(160 + 200 * lastStage, 450, "cursor").play(ANIMATION.CURSOR);
  }
  ...
  createStageClearReward() {
    ...
    if (lastStage === GAME.TOTAL_STAGE) {
      ...
      this.physics.add.collider(this.character, this.coin, () => {
        ...
        const heart = this.add.sprite(615, 300, "heart").setAlpha(0).setDepth(5);
